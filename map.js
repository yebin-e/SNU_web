...
(function(global){
  ...
  // 장르별 필터링된 도서관 표시
  function showFilteredLibraries(libraries, title) {
    if (!libraries || libraries.length === 0) {
      // 필터링된 도서관이 없으면 전체 도서관 표시
      showAllLibraries();
      return;
    }
    ...
    // 제목 표시 (선택사항)
    console.log(`지도에 표시: ${title}`);
  }
  
  // 모든 도서관 표시
  function showAllLibraries() {
    // 전체 도서관 데이터로 다시 렌더링
    if (window.allLibraries && window.allLibraries.length > 0) {
      render(window.allLibraries);
    }
  }

  global.MapView = { init, render, select, clear, on, showFilteredLibraries, showAllLibraries };
})(window);
